  ...
  this._moddle.fromXML(xml, 'bpmn:Definitions', function(err, definitions, context) {
    ...
    for(var i = 0; i < definitions.rootElements.length; i++){
      if(definitions.rootElements[i].$type === "bpmn:Process"){
        process = definitions.rootElements[i];
        break;
      }
    }
    var flows = process && process.flowElements;
    for (var i = 0; i < flows.length; i++) {
      if (flows[i].$type === "bpmn:SequenceFlow" || flows[i].$type === "bpmn:ParallelGateway"
        || flows[i].$type === "bpmn:StartEvent" || flows[i].$type === "bpmn:ExclusiveGateway"
        || flows[i].$type === "bpmn:EndEvent" ) {
        // console.info(fe);
        flows[i].name = "";
      }
    }
...
Viewer.prototype.saveXML = function(options, done) {
  ...
  console.info('save to xml ...');
  if(!definitions.$attrs['xmlns:bpmn2']){
    definitions.$attrs['xmlns:bpmn2']="http://www.omg.org/spec/BPMN/20100524/MODEL";
  }
  if(definitions.$attrs['xmlns']){
    delete definitions.$attrs['xmlns']
  }
